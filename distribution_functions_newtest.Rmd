...
y=rnorm(1000,-4,0.1) #stdeve as 0.25, 0.5, 1
delta<-10^(y) ##diffusion distance average as 100 um
std<-sd(delta)
...
for (i in 1:length(delta)){
  ...
  new<-solveCSS(data$mvol, data$po, data$bd, delta[i], data$soc, data$oxy, 1e-5, 1e-4,1.16e-6)#vmax=1 d-1
  #nvol<-new*delta[i]^3
...
maxc<-max(pred_sum$value)   #per colony
...
ava1<-solveCSS(data$mvol, data$po, data$bd, delta1, data$soc, data$oxy, 1e-5, 1e-4,1.16e-6)#vmax=1 d-1
